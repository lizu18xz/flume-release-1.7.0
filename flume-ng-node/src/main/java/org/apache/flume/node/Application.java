...
public class Application {
  ...
  //也是EventBus的回调函数，当eventbus执行post时，该方法就会被调用
  @Subscribe
  ...
  //按流程启动配置的组件  channel  sink  source
  ...
  public static void main(String[] args) {
    ...
    //flume-ng agent --conf conf -f conf/flume-kafka-conf.properties -n agent -Dflume.root.logger=INFO,console
    logger.info("启动flume服务!!!!!!!!");
    try {
      ...
      //如果不带--no-reload-conf这个参数，flume会自动加载配置参数 默认是30秒
